...
namespace Spline_005 {
    public partial class Form1 : Form {
		...
		public double SecondDevf(double x) {    // Вторые производные функций
            ...
            if (FuncFlag == 2) {
                return (2 - 3 * Math.Pow(x, 2.0)) / (Math.Pow(x, 3.0) * Math.Sqrt(Math.Pow((Math.Pow(x, 2.0) - 1), 3.0)));
            }
            if (FuncFlag == 3) {
        ...
        public double Spline(double x) {   // Сплайн
            for (int i = 1; i <= n; i++) {
                if ((x >= X[i - 1]) && (x <= X[i])) {
                    return (A[i] + B[i] * (x - X[i]) + C[i] * Math.Pow((x - X[i]), 2) + D[i] * Math.Pow((x - X[i]), 3));
                }
            }
            return (0.0);
        }

        public double SplineFirstDev(double x) {  // Первая производная сплайна
            for (int i = 1; i <= n; i++) {
                if ((x >= X[i - 1]) && (x <= X[i])) {
                    return (B[i] + 2 * C[i] * (x - X[i]) + 3 * D[i] * Math.Pow((x - X[i]), 2));
                }
            }
            return (0.0);
        }

        public double SplineSecondDev(double x) {   // Вторая производная сплайна
            for (int i = 1; i <= n; i++) {
                if ((x >= X[i - 1]) && (x <= X[i])) {
                    return (2 * C[i] + 6 * D[i] * (x - X[i]));
                }
            }
            return (0.0);
        }
        ...
        private void buttonGo_Click(object sender, EventArgs e) { // Вычисление коэффициентов сплайна
            n = System.Convert.ToInt32(textBoxN.Text);		// число разбиений
            ...
            for (int i = 1; i <= n - 1; i++) {
                ...
                // Betta[i + 1] = (  F[i] - betta[i]*A[i] ) / ( alpha[i] * A[i] + C[i] )
                // F[i] = 6 * (A[i + 1] - 2 * A[i] + A[i - 1]) / h;
                Betta[i + 1] = ((-6.0 / h) * (A[i + 1] - 2 * A[i] + A[i - 1]) + Betta[i] * h) / (-4 * h - Alpha[i] * h);
            }
			C[n] = myu2;
            for (int i = n; i >= 1; i--) {
            ...
            for (double x = a; x <= b; x += h / 4.0) {
            	//вычисление погрешности |F(x) - S(x)|
        ...
        public void Draw() { // Отрисовка графика
            GraphPane Pane = zedGraphControl1.GraphPane;
            if (GraphFlag == 1) { // График для функций
                if (FuncFlag == 1) {
                    Pane.Title = "f(x)={x^3+3x^2, -1<=x<=0;" + "\n" + "      {-x^3+3x^2, 0<=x<=1";
                }
                if (FuncFlag == 2) {
                    Pane.Title = "f(x)=√‎x^2+1/x, 1<=x<=Pi";
                }
                if (FuncFlag == 3) {
                    Pane.Title = "f(x)=sin(x+1)/x + cos(10x), 1<=x<=Pi";
                }
                if (FuncFlag == 4) {
                    Pane.Title = "f(x)=sin(x+1)/x + cos(100x), 1<=x<=Pi";
                }
            ...
            if (GraphFlag == 2) { // График для первых производных
                if (FuncFlag == 1) {
                    Pane.Title = "f'(x)";
                }
                if (FuncFlag == 2) {
            ...
            if (GraphFlag == 3) {  // График для вторых производных
                if (FuncFlag == 1) {
                    Pane.Title = "f''(x)";
                }
                if (FuncFlag == 2) {
